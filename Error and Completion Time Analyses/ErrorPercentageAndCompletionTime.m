            ...
            try
                ...
                for i = 1:4
                    ...
                    if (strcmp(tower, 'horizontal_left') || strcmp(tower, 'horizontal_right'))
                        onesegs = find(Segments == 1);
                        check = zeros(size(Segments));
                        check(onesegs) = 1;
                        [L, n] = bwlabel(check);

                        %remove the number of errors in the 1 segment
                        for j = 1:n1
                            ThisError = find(L1 == j);
                            first = ThisError(1);
                            last = ThisError(end);
                            for h = 1:n % run over the 1 segmnets
                                ThisSeg = find(L == h);
                                first2 = ThisSeg(1);
                                last2 = ThisSeg(end);
                                %if the entire error is contained within a
                                %1 segment -> delete it (segment starts
                                %before error starts & ends after error
                                %ends
                        ...
                        TotalStamps = TotalStamps - length(onesegs);

                        MoveInds = find(Moves == 1);
                        MoveOneInds = intersect(MoveInds, onesegs);
                        ErrorStamps = ErrorStamps - length(MoveOneInds);
                        %number of timestamps are updated, now update time.
                        % need to take into account that there might be
                        % more than one 1 segment, if it was entered and
                        % exited
                        ...
                        for j = 1:n
                            ...
                            first = ThisSeg(1);
                            last = ThisSeg(end);
                            segtime = segtime + (VidTimeStamps(last + start_ind - 1) - VidTimeStamps(first + start_ind - 1));

                            moveoneinds = intersect(MoveInds, ThisSeg); %this shows the intersection between this one segment and the moves
                            check2 = zeros(size(Moves));
                            check2(moveoneinds) = 1;
                            [LL, nn] = bwlabel(check2);


                            for jj = 1:nn
                                ThisError = find(LL == jj);
                                first = ThisError(1);
                                last = ThisError(end);
                                errsegtime = errsegtime + (VidTimeStamps(last + start_ind - 1) - VidTimeStamps(first + start_ind - 1));
                            end
                        end
                        if errsegtime > segtime
                            warning(['errsegtime > segtime ', participant, month, tt, ' ', tower])
                        end
                        TotalTime = TotalTime - segtime;
                        ErrorTime = ErrorTime - errsegtime;

                        if ErrorTime > TotalTime
                            warning(['ErrorTime > TotalTime ', participant, month, tt, ' ', tower])
                        end
                    end % done removing segment 1 from horizontal towers
                   ...
                   if exist('ShutDowns','var')
                        if (any(ShutDowns > start_ind) && any(ShutDowns < end_ind))
                            ...
                            if Restarts(ind) < end_ind
                                ...
                                if length(unique(Segments(shutdowninds - start_ind + 1))) > 1
                                    warning(['Multiple segments in shut down ', participant, month, tt, ' ', tower])
                                end
                                if (strcmp(tower, 'horizontal_left') || strcmp(tower, 'horizontal_right'))
                                    if unique(Segments(shutdowninds - start_ind + 1)) == 1
                                        %then these stamps should have already
                                        %been removed

                                        %ensure that they are all errors
                                        assert(all(Moves(shutdowninds - start_ind + 1)) == 1)
